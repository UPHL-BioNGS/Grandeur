process QUAST {
  ...
  output:
  path "quast/*", emit: files, optional: true
  ...
  script:
  def args   = task.ext.args   ?: '--no-icarus'
  def prefix = task.ext.prefix ?: "${meta.id}"
  def fastq  = reads[1] ? "--pe1 ${reads[0]} --pe2 ${reads[1]}" : ""
  def fin    = reads[1] ? "quast/${prefix}_quast_transposed_report.tsv" : "quast/${prefix}_quast_transposed_report_contig.tsv"
  """
