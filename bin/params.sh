...
do
    ...
    config_check=$(grep -w "^//params.$param" configs/grandeur_template.config | head -n 1)
    if [ -n "$config_check" ]
    then
        # 4. checks that every param and its default value is in the config file
        config_default="$(grep  -w "^//params.$param" configs/grandeur_template.config | grep -v "#" | head -n 1 | sed 's/.*=//g' | awk '{ ( $1=$1 ) ; print $0 }' )"
        echo -e "config:\t\t$config_default"
       
    else
        echo "$param was not included in configs/grandeur_template.config file!!!"
    fi
